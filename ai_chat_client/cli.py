...
from ai_chat_client.arms.mcp import _exception_summary, run_mcp_case
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare baseline and MCP LLM experiment arms."
    )
...
async def run() -> None:
    ...
    results = []
    baseline_schema = (
        settings.baseline_schema() if settings.experimental_arm == "BASELINE" else None
    )

    print(
        f"Starting {settings.experimental_arm} run with {len(test_cases)} questions using {settings.llm_provider}/{settings.llm_model}."
    )
    for i, test_case in enumerate(test_cases):
        ...
        if settings.question_delay_seconds > 0 and i < len(test_cases) - 1:
            print(f"Waiting {settings.question_delay_seconds}s before next question...")
            await asyncio.sleep(settings.question_delay_seconds)
...
async def doctor() -> None:
    ...
    headers = (
        {"Authorization": settings.mcp_auth_header} if settings.mcp_auth_header else {}
    )
    try:
